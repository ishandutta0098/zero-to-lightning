...
print("----------------------------------")
trainer = pl.Trainer(max_epochs=1, default_root_dir="../../experiments/", callbacks=[EarlyStopping(monitor="val_loss", mode="min"), ModelSummary(max_depth=-1)])
trainer.fit(model, train_dataloader, val_dataloader)
...
print("----------------------------------")
trainer = pl.Trainer(max_epochs=1, default_root_dir="../../experiments/", callbacks=[EarlyStopping(monitor="val_loss", mode="min")], enable_model_summary=False)
trainer.fit(model, train_dataloader, val_dataloader)